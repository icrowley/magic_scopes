...
Code speaks much louder than words, so I show you some examples at first.
...
* You can specify in (stands for include) and ex (for exclude) options, so:
  ...
  * Also you can specify scope name for scope type per attribute, like so:
      magic_scopes :title, rating: {by_desc: :by_popularity}
    It will generate all possible scopes for title attribute and by_popularity scope that sorts relation by rating, desc.
